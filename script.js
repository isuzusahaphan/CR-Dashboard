// ⚠️ อย่าลืมแก้ API_URL เป็น URL ที่ได้จากการ Deploy Code.gs เวอร์ชันใหม่ล่าสุดนะครับ
...
// ตัวแปรสำหรับระบบศูนย์ฝึกอบรม (Training)
...
function loadDashboard() {
    ...
    .then(r => r.json()).then(res => {
        ...
        if (res.result === 'success') {
            ...
            let motivationText = "";
            if(percent >= 100) { pb.style.background = "linear-gradient(90deg, #1b5e20, #388e3c)"; motivationText = "🏆 ยอดเยี่ยมเหนือความคาดหมาย! ผลงานทะลุเป้าหมายประจำเดือนแล้ว ขอเสียงปรบมือให้ทีม CR ครับ 🎉"; }
            else if(percent >= 80) { pb.style.background = "linear-gradient(90deg, #388e3c, #81c784)"; motivationText = "🔥 โค้งสุดท้ายแล้ว! ผลงานทะลุ 80% ลุยอีกนิดเดียวเป้าหมายอยู่แค่เอื้อมครับ 🚀"; }
            else if(percent >= 50) { pb.style.background = "linear-gradient(90deg, #1565c0, #4fc3f7)"; motivationText = "💪 เดินทางมาเกินครึ่งทางแล้ว! รักษามาตรฐานการทำงานที่ยอดเยี่ยมนี้ต่อไปครับ ✨"; }
            else { pb.style.background = "linear-gradient(90deg, #d32f2f, #e57373)"; motivationText = "🌱 เริ่มต้นเป้าหมายใหม่ ค่อยๆ สะสมยอดไปทีละคัน เป็นกำลังใจให้ทีม CR ทุกคนครับ ✌️"; }
            ...
            typeChartInstance = new Chart(document.getElementById('typeChart'), {
                ...
                options: { responsive: true, maintainAspectRatio: false, cutout: '55%', plugins: { legend: { position: 'right', labels: { boxWidth: 12, font: { size: 13 } } }, datalabels: { color: '#fff', font: { weight: 'bold', size: 13 }, textAlign: 'center', formatter: donutFormatter } } }
            });
            ...
            if (d.csvData && d.csvData.length > 0) {
                ...
                d.csvData.forEach(item => {
                    ...
                    csvHtml += `<div class="csv-card"><div class="csv-title">${item.group}</div><div style="display:flex; justify-content:space-between; margin-bottom:8px; font-size:14px;"><span style="color:#555;">ความคืบหน้าการโทร</span><span style="color:#1565c0; font-weight:bold;">${actualPercent}%</span></div><div style="width:100%; background:#e0e0e0; height:12px; border-radius:6px; overflow:hidden; margin-bottom:15px;"><div style="width:${actualPercent}%; background:linear-gradient(90deg, #1976d2, #4fc3f7); height:100%; transition: width 1s;"></div></div><div style="display:flex; justify-content:space-between; font-size:13px; color:#555; background:#f9f9f9; padding:8px; border-radius:6px; margin-bottom:5px;"><span>🎯 เป้าหมายกลุ่ม:</span><b>${item.target} คัน</b></div><div style="display:flex; justify-content:space-between; font-size:13px; color:#555; background:#f9f9f9; padding:8px; border-radius:6px; margin-bottom:5px;"><span>✅ เข้าก่อนติดตาม:</span><b style="color:#2e7d32;">${item.preService} คัน</b></div><div style="display:flex; justify-content:space-between; font-size:13px; color:#555; background:#f9f9f9; padding:8px; border-radius:6px;"><span>📞 สถานะการโทร:</span><span>โทรแล้ว <b style="color:#1565c0;">${item.tracked}</b> | ค้าง <b style="color:#d32f2f;">${item.untracked}</b></span></div></div>`;
                });
                document.getElementById('dash_csv_tracking').innerHTML = csvHtml;
            } else { document.getElementById('dash_csv_tracking').innerHTML = '<p style="grid-column: 1 / -1; text-align:center; color:#777; padding: 40px; background: #fff; border: 1px dashed #ccc; border-radius: 10px;">⚠️ ผู้ดูแลระบบยังไม่ได้อัปโหลดฐานข้อมูลการติดตามลูกค้าในเดือนนี้</p>'; }
        }
...
function openBreakdownModal() {
    ...
    setTimeout(() => {
        ...
        kannikaChartInstance = new Chart(document.getElementById('kannikaChart'), { type: 'doughnut', data: { labels: ['ระบบตรีเพชร', 'ติดต่อด้วยตนเอง', 'Walk-in/แนะนำ'], datasets: [{ data: [d.kannikaBreakdown.tripetch, d.kannikaBreakdown.inbound, d.kannikaBreakdown.referral], backgroundColor: donutColors, borderWidth: 2 }] }, options: { responsive: true, maintainAspectRatio: false, cutout: '55%', plugins: { legend: { position: 'bottom', labels: { boxWidth: 12, font: { size: 12 } } }, datalabels: { color: '#fff', font: { weight: 'bold', size: 12 }, textAlign: 'center', formatter: donutFormatter } } } });
        if (ruangsiriChartInstance) ruangsiriChartInstance.destroy();
        ruangsiriChartInstance = new Chart(document.getElementById('ruangsiriChart'), { type: 'doughnut', data: { labels: ['ระบบตรีเพชร', 'ติดต่อด้วยตนเอง', 'Walk-in/แนะนำ'], datasets: [{ data: [d.ruangsiriBreakdown.tripetch, d.ruangsiriBreakdown.inbound, d.ruangsiriBreakdown.referral], backgroundColor: donutColors, borderWidth: 2 }] }, options: { responsive: true, maintainAspectRatio: false, cutout: '55%', plugins: { legend: { position: 'bottom', labels: { boxWidth: 12, font: { size: 12 } } }, datalabels: { color: '#fff', font: { weight: 'bold', size: 12 }, textAlign: 'center', formatter: donutFormatter } } } });
    }, 100);
}
function closeBreakdownModal() { document.getElementById('breakdown-modal').style.display = 'none'; }
...
function saveRecord() {
    ...
    const payload = { action: "save_record", date: recordDate, cr_name: crName, tripetch: parseInt(document.getElementById('type_tripetch').value) || 0, inbound: parseInt(document.getElementById('type_inbound').value) || 0, referral: parseInt(document.getElementById('type_referral').value) || 0, total: tot };
...
// ==========================================
// 🎓 แท็บ 3: ศูนย์ฝึกอบรมและประเมินผล (Training)
// ==========================================
...
function prepareQuiz() {
    ...
    .then(r => r.json()).then(res => {
        ...
        if(res.result === 'success' && res.data.length > 0) {
            ...
            showQuestion();
        } else alert("❌ ไม่พบคำถามในหัวข้อนี้ กรุณาติดต่อผู้ดูแลระบบ");
    }).catch(e => {
        document.getElementById('loading-overlay').style.display = 'none';
...
function showQuestion() {
    ...
    let html = `<p class="quiz-q-text">${currentQuestionIndex + 1}. ${q.q}</p><div class="quiz-options">
        <div class="quiz-opt" onclick="selectOption('A')"><span style="background:#eee; padding:5px 10px; border-radius:5px;">A</span> ${q.a}</div>
        <div class="quiz-opt" onclick="selectOption('B')"><span style="background:#eee; padding:5px 10px; border-radius:5px;">B</span> ${q.b}</div>
        <div class="quiz-opt" onclick="selectOption('C')"><span style="background:#eee; padding:5px 10px; border-radius:5px;">C</span> ${q.c}</div>
        <div class="quiz-opt" onclick="selectOption('D')"><span style="background:#eee; padding:5px 10px; border-radius:5px;">D</span> ${q.d}</div>
    </div>`;
...
function resetQuiz() {
    ...
    loadScoreHistory(); // โหลดประวัติใหม่เพื่อให้เห็นคะแนนที่เพิ่งสอบไป
}

// ==========================================
// 🌟 แท็บ 4: รายงานสรุป (Reports & Filters)
// ==========================================
        ...
        complete: function(results) {
            ...
            .then(r => r.json()).then(res => {
                ...
                if(res.result === 'success') { 
                    ...
                    fileInput.value = ""; 
                    document.getElementById('dash_month').value = adminMonth; document.getElementById('dash_year').value = adminYear;
